...
input long   MagicNumber                    = 25052302;
...
input int    D1LineLookbackBars             = 160;
input int    H4LineLookbackBars             = 240;
input int    MinLineTouches                 = 3;
input double LineTolerancePips              = 5.0;
input double BreakoutBufferPips             = 1.0;
input double MinBreakoutBodyPips            = 6.0;
input double BigCandleAtrMultiplier         = 1.20;
input double MinATRPips                     = 4.0;
input double StopBufferPips                 = 2.0;
input double MinStopLossPips                = 5.0;
input double MaxStopLossPips                = 20.0;
input double RewardRiskRatio                = 2.0;
input int    SlippagePoints                 = 20;
input int    ManagementTimerSeconds         = 30;
input string OrderComment                   = "USDJPY_SR_Breakout_RiskGuard";
...
void InitBreakoutSetup(BreakoutSetup &setup);
bool FindLineFromTimeframe(const ENUM_TIMEFRAMES timeframe,
                           const int lookback_bars,
                           const bool bullish,
                           const double previous_close,
                           const double breakout_close,
                           double &line_price,
                           int &touches,
                           string &reason);
bool FindBestBreakoutLine(const bool bullish,
                          const double previous_close,
                          const double breakout_close,
                          double &line_price,
                          string &line_timeframe,
                          int &touches,
                          string &reason);
bool EvaluateBreakoutSignal(const MarketSnapshot &snapshot, BreakoutSetup &setup, string &reason);
bool ValidateVolume(double &volume, string &reason);
bool PrepareOrderPrices(const BreakoutSetup &setup, double &entry_price, double &stop_loss, double &take_profit, string &reason);
bool PlaceEntryOrder(const BreakoutSetup &setup);
...
string FormatMarketSummary(const MarketSnapshot &snapshot);
...
int OnInit()
{
   ...
   {
      PrintFormat("[INIT] ATR handle creation failed. handle=%d", g_handle_m15_atr14);
      return(INIT_FAILED);
   }
...
void ProcessNewM15Bar()
{
   ...
   if(EvaluateBreakoutSignal(snapshot, setup, signal_reason))
   {
      PlaceEntryOrder(setup);
      return;
   }

   PrintFormat("[NO_ENTRY] No valid SR breakout. reason=%s | %s",
               signal_reason,
               FormatMarketSummary(snapshot));
}
...
bool LoadMarketSnapshot(MarketSnapshot &snapshot, string &reason)
{
   ...
   if(CopyBuffer(g_handle_m15_atr14, 0, 0, 3, m15_atr14) < 3)
   {
...
{
   setup.valid = false;
   setup.order_type = ORDER_TYPE_BUY;
   setup.line_price = 0.0;
   setup.line_timeframe = "";
   setup.line_touches = 0;
                           ...
                           string &reason)
{
   ...
   int copied = CopyRates(_Symbol, timeframe, 1, lookback_bars, rates);
   if(copied < MinLineTouches)
   {
      reason = StringFormat("%s has too few bars for line detection.", EnumToString(timeframe));
      return(false);
   }

   double tolerance = LineTolerancePips * PipSize();
   double breakout_buffer = BreakoutBufferPips * PipSize();
   double best_score = -1000000000.0;

   for(int index = 0; index < copied; ++index)
   {
      double body_high = MathMax(rates[index].open, rates[index].close);
      double body_low = MathMin(rates[index].open, rates[index].close);
      double candidate = (bullish ? body_high : body_low);
      ...
      if(bullish)
      {
         if(previous_close > candidate || breakout_close < candidate + breakout_buffer)
            continue;
      }
      else
      {
      ...
      int count = 0;
      double sum = 0.0;
      for(int touch_index = 0; touch_index < copied; ++touch_index)
      {
         double touch_body_high = MathMax(rates[touch_index].open, rates[touch_index].close);
         double touch_body_low = MathMin(rates[touch_index].open, rates[touch_index].close);
         double touch_level = (bullish ? touch_body_high : touch_body_low);

         if(MathAbs(touch_level - candidate) <= tolerance)
         {
            count++;
            sum += touch_level;
         }
      ...
      if(count < MinLineTouches)
         continue;

      double averaged_line = sum / count;
      if(bullish)
      {
         if(previous_close > averaged_line || breakout_close < averaged_line + breakout_buffer)
            continue;
      }
      else
      {
      ...
      double distance_pips = MathAbs(breakout_close - averaged_line) / PipSize();
      double score = count * 1000.0 - distance_pips;
      if(score > best_score)
      {
         best_score = score;
         line_price = averaged_line;
         touches = count;
      }
   }

   if(touches < MinLineTouches)
   {
      reason = StringFormat("%s found no valid %s line.",
                            EnumToString(timeframe),
                            (bullish ? "resistance breakout" : "support breakout"));
      return(false);
   }
                          ...
                          string &reason)
{
   ...
   bool has_d1 = FindLineFromTimeframe(PERIOD_D1, D1LineLookbackBars, bullish, previous_close, breakout_close, d1_line, d1_touches, d1_reason);
   bool has_h4 = FindLineFromTimeframe(PERIOD_H4, H4LineLookbackBars, bullish, previous_close, breakout_close, h4_line, h4_touches, h4_reason);

   if(!has_d1 && !has_h4)
   {
      reason = StringFormat("%s | %s", d1_reason, h4_reason);
      return(false);
   }

   if(has_d1 && (!has_h4 || d1_touches >= h4_touches))
   {
      line_price = d1_line;
      line_timeframe = "D1";
      touches = d1_touches;
...
bool EvaluateBreakoutSignal(const MarketSnapshot &snapshot, BreakoutSetup &setup, string &reason)
{
   InitBreakoutSetup(setup);
   reason = "";

   double pip = PipSize();
   double body_pips = MathAbs(snapshot.m15_bar_1.close - snapshot.m15_bar_1.open) / pip;
   double atr_pips = snapshot.m15_atr14 / pip;
   double required_body_pips = MathMax(MinBreakoutBodyPips, atr_pips * BigCandleAtrMultiplier);

   if(body_pips < required_body_pips)
   {
      reason = StringFormat("M15 body is too small: %.2f pips < %.2f pips.", body_pips, required_body_pips);
      return(false);
   }

   bool bullish_candle = (snapshot.m15_bar_1.close > snapshot.m15_bar_1.open);
   bool bearish_candle = (snapshot.m15_bar_1.close < snapshot.m15_bar_1.open);
   if(!bullish_candle && !bearish_candle)
   {
      reason = "M15 breakout candle has no directional body.";
      return(false);
   }
   ...
   if(bullish_candle)
   {
      if(!FindBestBreakoutLine(true, snapshot.m15_bar_2.close, snapshot.m15_bar_1.close, line, line_tf, line_touches, line_reason))
      {
         reason = "Buy rejected: " + line_reason;
         return(false);
      }

      setup.valid = true;
      setup.order_type = ORDER_TYPE_BUY;
      setup.line_price = line;
      setup.line_timeframe = line_tf;
      setup.line_touches = line_touches;
      setup.stop_reference = MathMin(line - StopBufferPips * pip, snapshot.m15_bar_1.low - StopBufferPips * pip);
      setup.body_pips = body_pips;
      setup.atr_pips = atr_pips;
      setup.reason = StringFormat("BUY SR breakout. %s line=%.3f touches=%d M15 close=%.3f body=%.2f ATR=%.2f",
                                  line_tf,
                                  line,
                                  line_touches,
                                  snapshot.m15_bar_1.close,
                                  body_pips,
                                  atr_pips);
      reason = setup.reason;
      return(true);
   ...
   if(!FindBestBreakoutLine(false, snapshot.m15_bar_2.close, snapshot.m15_bar_1.close, line, line_tf, line_touches, line_reason))
   {
      reason = "Sell rejected: " + line_reason;
      return(false);
   }

   setup.valid = true;
   setup.order_type = ORDER_TYPE_SELL;
   setup.line_price = line;
   setup.line_timeframe = line_tf;
   setup.line_touches = line_touches;
   setup.stop_reference = MathMax(line + StopBufferPips * pip, snapshot.m15_bar_1.high + StopBufferPips * pip);
   setup.body_pips = body_pips;
   setup.atr_pips = atr_pips;
   setup.reason = StringFormat("SELL SR breakout. %s line=%.3f touches=%d M15 close=%.3f body=%.2f ATR=%.2f",
                               line_tf,
                               line,
                               line_touches,
                               snapshot.m15_bar_1.close,
...
bool PrepareOrderPrices(const BreakoutSetup &setup, double &entry_price, double &stop_loss, double &take_profit, string &reason)
{
   ...
   if(!setup.valid)
   {
      reason = "Breakout setup is invalid.";
      return(false);
   }
   ...
   double pip = PipSize();
   if(setup.order_type == ORDER_TYPE_BUY)
   {
      entry_price = tick.ask;
      stop_loss = NormalizeDouble(setup.stop_reference, _Digits);
      if(stop_loss >= entry_price)
      {
         reason = "Buy stop loss is not below entry price.";
         return(false);
      }

      double stop_pips = (entry_price - stop_loss) / pip;
      if(stop_pips < MinStopLossPips || stop_pips > MaxStopLossPips)
      {
         reason = StringFormat("Buy stop distance is outside limits: %.2f pips.", stop_pips);
         return(false);
      }

      take_profit = NormalizeDouble(entry_price + stop_pips * RewardRiskRatio * pip, _Digits);
   }
   else if(setup.order_type == ORDER_TYPE_SELL)
   {
      entry_price = tick.bid;
      stop_loss = NormalizeDouble(setup.stop_reference, _Digits);
      if(stop_loss <= entry_price)
      {
         reason = "Sell stop loss is not above entry price.";
         return(false);
      }

      double stop_pips = (stop_loss - entry_price) / pip;
      if(stop_pips < MinStopLossPips || stop_pips > MaxStopLossPips)
      {
         reason = StringFormat("Sell stop distance is outside limits: %.2f pips.", stop_pips);
         return(false);
      }
   ...
   if(min_stop_distance > 0.0)
   {
      ...
      {
         reason = StringFormat("Broker stop level blocks SL/TP. stop_level=%.5f actual_sl=%.5f actual_tp=%.5f",
                               min_stop_distance,
...
bool PlaceEntryOrder(const BreakoutSetup &setup)
{
   ...
   string price_reason = "";
   if(!PrepareOrderPrices(setup, entry_price, stop_loss, take_profit, price_reason))
   {
   ...
   double stop_pips = MathAbs(entry_price - stop_loss) / PipSize();
   double take_pips = MathAbs(take_profit - entry_price) / PipSize();
   PrintFormat("[ENTRY] type=%s volume=%.4f price=%.3f sl=%.3f tp=%.3f stop=%.2f tp=%.2f rr=%.2f ticket=%I64u reason=%s",
               EnumToString(setup.order_type),
               volume,
               entry_price,
               stop_loss,
...
string FormatMarketSummary(const MarketSnapshot &snapshot)
{
   double body_pips = MathAbs(snapshot.m15_bar_1.close - snapshot.m15_bar_1.open) / PipSize();
   return(StringFormat("spread=%.2f ATR=%.2f body=%.2f prevClose=%.3f close=%.3f",
                       snapshot.spread_pips,
                       snapshot.m15_atr14 / PipSize(),
                       body_pips,
                       snapshot.m15_bar_2.close,
                       snapshot.m15_bar_1.close));
}
...
bool ValidateInputs(string &reason)
{
   ...
   if(D1LineLookbackBars < MinLineTouches || H4LineLookbackBars < MinLineTouches)
   {
      reason = "Line lookback bars must be greater than or equal to MinLineTouches.";
      return(false);
   }

   if(MinLineTouches < 2)
   {
      reason = "MinLineTouches must be at least 2.";
      return(false);
   }

   if(LineTolerancePips <= 0.0 || BreakoutBufferPips <= 0.0)
   {
      reason = "LineTolerancePips and BreakoutBufferPips must be greater than zero.";
      return(false);
   }

   if(MinBreakoutBodyPips <= 0.0 || BigCandleAtrMultiplier <= 0.0 || MinATRPips < 0.0)
   {
      reason = "Breakout candle and ATR inputs must be valid positive values.";
      return(false);
   }

   if(StopBufferPips < 0.0 || MinStopLossPips <= 0.0 || MaxStopLossPips <= 0.0)
   {
      reason = "Stop loss inputs must be valid positive values.";
      return(false);
   }

   if(MinStopLossPips > MaxStopLossPips)
   {
      reason = "MinStopLossPips must not exceed MaxStopLossPips.";
      return(false);
   }

   if(RewardRiskRatio < 1.0)
   {
      reason = "RewardRiskRatio must be at least 1.0.";
      return(false);
   }
