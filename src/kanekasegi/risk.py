...
class RiskEngine:
    ...
    def size_position(self, entry_price: float, stop_price: float, equity: float) -> float:
        ...
        stop_distance = abs(entry_price - stop_price) * self.config.contract_point_value
        if stop_distance <= 0:
            raise ValueError("stop distance must be positive")
        quantity = risk_amount / stop_distance
        if self.config.max_position_notional is not None:
            entry_notional = entry_price * self.config.contract_point_value
            quantity = min(quantity, self.config.max_position_notional / entry_notional)
        if self.config.per_contract_margin is not None:
