...
### 1. day_breakout_opening_midweek_wide_trail
...
### 3. day_breakout_opening_midweek_tight_exit
...
### 4. day_breakout_opening_midweek_long_only
...
### 5. day_breakout_morning_midweek
...
- `day_breakout_opening_midweek_wide_trail` is the new leader. It improved profit by about `4.0k` over the prior midweek opening rule while keeping drawdown effectively flat.
...
- The current practical shortlist is:
  ...
  - `day_breakout_morning_midweek`
- The next research step should stay inside the opening Tue-Thu regime and test smaller exit variations or an explicit short-side suppression rule instead of pure short-only isolation.
